...
btnFinish.addEventListener("click", finishFlightPlan);
...
async function importFromStorage() {
    ...
    if (window.location.href.split("/").includes("edit")) {
        ...
        const data = document.getElementById('map').getAttribute('flightplan');
        if (!data) {
            return;
        }
...
window.addEventListener("message", (event) => {
    if (event.origin === window.location.origin) {
        if (event.data.case === "visualization") {
...
function createMultiFile(singlePathData) {
