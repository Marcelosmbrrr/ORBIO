...
import { ShowEquipment } from '@/Components//Equipments/ShowEquipment';
...
export default function Equipments() {
    ...
    const equipments: EquipmentRecord[] = data.data;
    ...
    const toggleSelection = (recordId: string) => {
        setSelections(prev => {
            ...
            if (exists) {
                ...
                const equipment = equipments.find(equipment => equipment.id === recordId);
                if (equipment) {
        ...
        <AuthenticatedLayout>
            <div className='flex flex-col h-full'>
                ...
                <div className='grow py-5 rounded'>
                    ...
                    <EquipmentTable equipments={equipments} isSelected={isSelected} toggleSelection={toggleSelection} />
...
const ActionButtons = ({ canCreate, canEdit, canShow, canDeleteOrUndelete, selections, reload, currentParams }: any) => (
    <div className="flex justify-start md:justify-end flex-shrink-0 w-full md:w-auto md:flex-row md:space-y-0 md:items-center space-x-1">
        <CreateEquipment can_open={canCreate} />
        <EditEquipment can_open={canEdit} selection={selections[0]} />
        <ShowEquipment can_open={canShow} selection={selections[0]} />
        <DeleteOrUndeleteResource
            can_open={canDeleteOrUndelete}
            reload={reload}
            action={currentParams.group === "deleted" || selections.every((sel: EquipmentSelected) => sel.is_deleted) ? "undelete" : "delete"}
            request_url={currentParams.group === "deleted" || selections.every((sel: EquipmentSelected) => sel.is_deleted)
                ? `/actions/undelete/equipments?ids=${selections.map((selection: any) => selection.id).join(',')}`
                : `/equipments/delete-many?ids=${selections.map((selection: any) => selection.id).join(',')}`
            }
        />
...
const EquipmentTable = ({ equipments, isSelected, toggleSelection }: any) => (
    <div className="mt-2 overflow-x-auto sm:rounded-lg">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                {equipments.length > 0 ? equipments.map((equipment: EquipmentRecord) => (
                        ...
                        <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            <div className="flex items-center">
                                <input
                                    ...
                                    onChange={() => toggleSelection(equipment.id)}
                                    value={equipment.id}
                                    type="checkbox"
