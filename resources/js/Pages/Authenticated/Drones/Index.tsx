...
export default function Drones() {
    ...
    const drones: DroneRecord[] = pagination.data;
    ...
    <div className="flex justify-start md:justify-end flex-shrink-0 w-full md:w-auto md:flex-row md:space-y-0 md:items-center space-x-1">
        ...
        <DeleteOrUndeleteResource
            ...
            action={
                ...
                selections.every((sel: DroneSelected) => sel.is_deleted)
                    ? "undelete"
                    : "delete"
            }
            request_url={
                currentParams.group === "deleted" ||
                selections.every((sel: DroneSelected) => sel.is_deleted)
                    ? `/actions/undelete/drones?ids=${selections
                          .map((selection: any) => selection.id)
                          .join(",")}`
                    : `/drones/delete-many?ids=${selections
                          .map((selection: any) => selection.id)
                          .join(",")}`
            }
        />
...
const DroneTable = ({ drones, isSelected, toggleSelection, canEdit }: any) => (
                    ...
                    drones.map((drone: DroneRecord) => (
                        ...
                        >
                            ...
                            <td className="flex justify-end gap-x-2 text-left px-6 py-4">
                                <EditDrone id={drone.id} can_open={canEdit} />
                                <ShowDrone id={drone.id} />
                            </td>
                        </tr>
                    ))
                ) : (
