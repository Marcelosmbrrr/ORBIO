...
import { CreateDrone } from '@/Components/Drones/CreateDrone';
import { EditDrone } from '@/Components/Drones/EditDrone';
import { ShowDrone } from '@/Components/Drones/ShowDrone';
...
// Types
import { DroneSelected, DroneRecord } from './types';
...
export default function Drones() {
    ...
    const drones: DroneRecord[] = data.data;
    ...
    const handleNavigation = React.useCallback((params: Partial<QueryParams>) => {
        setSelections([]);
        router.get("drones", { ...currentParams, ...params });
    }, [currentParams]);
    ...
    const toggleSelection = (recordId: string) => {
        setSelections(prev => {
            ...
            if (exists) {
                ...
                const drone = drones.find(drone => drone.id === recordId);
                if (drone) {
                    const newSelection: DroneSelected = {
                        id: drone.id,
                        is_deleted: Boolean(drone.deleted_at)
                    };
    ...
    const canOpenDeleteOrUndelete = selections.length > 0 && auth.user.authorization.equipments.write && selections.every(sel => sel.is_deleted === selections[0].is_deleted);
            ...
            <div className='flex flex-col h-full'>
                ...
                <div className='grow py-5 rounded'>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center space-y-3 md:space-y-0">
                        ...
                        <ActionButtons
                            canCreate={selections.length === 0 && auth.user.authorization.equipments.write}
                            canEdit={selections.length === 1 && auth.user.authorization.equipments.write}
                            canShow={selections.length === 1 && auth.user.authorization.equipments.write}
                    ...
                    <DroneTable drones={drones} isSelected={isSelected} toggleSelection={toggleSelection} />
...
const ActionButtons = ({ canCreate, canEdit, canShow, canDeleteOrUndelete, selections, reload, currentParams }: any) => (
    <div className="flex justify-start md:justify-end flex-shrink-0 w-full md:w-auto md:flex-row md:space-y-0 md:items-center space-x-1">
        <CreateDrone can_open={canCreate} />
        <EditDrone can_open={canEdit} selection={selections[0]} />
        <ShowDrone can_open={canShow} selection={selections[0]} />
        <DeleteOrUndeleteResource
            can_open={canDeleteOrUndelete}
            reload={reload}
            action={currentParams.group === "deleted" || selections.every((sel: DroneSelected) => sel.is_deleted) ? "undelete" : "delete"}
            request_url={currentParams.group === "deleted" || selections.every((sel: DroneSelected) => sel.is_deleted)
                ? `/actions/undelete/drones?ids=${selections.map((selection: any) => selection.id).join(',')}`
                : `/drones/delete-many?ids=${selections.map((selection: any) => selection.id).join(',')}`
            }
        />
        <LimitSelector value={currentParams.limit} changeLimit={(limit: string) => reload({ limit })} />
        <OrderSelector value={currentParams.order_by} options={[{ id: "id", name: "criação" }, { id: "name", name: "nome" }, { id: "manufacturer", name: "fabricante" }, { id: "model", name: "modelo" }, { id: "weight", name: "peso" }]} changeOrderBy={(order_by: string) => reload({ order_by })} />
    </div>
);
...
const DroneTable = ({ drones, isSelected, toggleSelection }: any) => (
    <div className="mt-2 overflow-x-auto sm:rounded-lg">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                {drones.length > 0 ? drones.map((drone: DroneRecord) => (
                    <tr key={drone.id} className="bg-white dark:text-white border-b dark:bg-gray-900 dark:border-gray-700 hover:bg-gray-50">
                        <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            <div className="flex items-center">
                                <input
                                    checked={isSelected(drone.id)}
                                    onChange={() => toggleSelection(drone.id)}
                                    value={drone.id}
                                    type="checkbox"
                        ...
                        <td className="px-6 py-4">
                            <div className="flex items-center text-gray-900 dark:text-white">
                                <div className={statusClassname[drone.status.style_key]}></div> {drone.status.title}
                            </div>
                        </td>
                        <td className="text-left px-6 py-4">{drone.name}</td>
                        <td className="text-left px-6 py-4">{drone.manufacturer}</td>
                        <td className="text-left px-6 py-4">{drone.model}</td>
                        <td className="text-left px-6 py-4">{drone.record_number}</td>
                        <td className="text-left px-6 py-4">{drone.serial_number}</td>
                        <td className="text-left px-6 py-4">{drone.weight}</td>
                    </tr>
                )) : (
