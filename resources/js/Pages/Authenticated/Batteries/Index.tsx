...
import { EditBattery } from '@/Components/Batteries/EditBattery';
import { ShowBattery } from '@/Components/Batteries/ShowBattery';
    ...
    const toggleSelection = (recordId: string) => {
        setSelections(prev => {
            ...
            if (exists) {
                ...
                const battery = batteries.find(battery => battery.id === recordId);
                if (battery) {
        ...
        <AuthenticatedLayout>
            <div className='flex flex-col h-full'>
                ...
                <div className='grow py-5 rounded'>
                    ...
                    <BatteryTable batteries={batteries} isSelected={isSelected} toggleSelection={toggleSelection} />
...
const ActionButtons = ({ canCreate, canEdit, canShow, canDeleteOrUndelete, selections, reload, currentParams }: any) => (
    <div className="flex justify-start md:justify-end flex-shrink-0 w-full md:w-auto md:flex-row md:space-y-0 md:items-center space-x-1">
        <CreateBattery can_open={canCreate} />
        <EditBattery can_open={canEdit} selection={selections[0]} />
        <ShowBattery can_open={canShow} selection={selections[0]} />
        <DeleteOrUndeleteResource
            can_open={canDeleteOrUndelete}
            reload={reload}
            action={currentParams.group === "deleted" || selections.every((sel: BatterySelected) => sel.is_deleted) ? "undelete" : "delete"}
            request_url={currentParams.group === "deleted" || selections.every((sel: BatterySelected) => sel.is_deleted)
                ? `/actions/undelete/batteries?ids=${selections.map((selection: any) => selection.id).join(',')}`
                : `/batteries/delete-many?ids=${selections.map((selection: any) => selection.id).join(',')}`
            }
        />
        <LimitSelector value={currentParams.limit} changeLimit={(limit: string) => reload({ limit })} />
        <OrderSelector value={currentParams.order_by} options={[{ id: "id", name: "criação" }, { id: "name", name: "nome" }, { id: "manufacturer", name: "fabricante" }, { id: "last_charge", name: "última carga" }]} changeOrderBy={(order_by: string) => reload({ order_by })} />
    </div>
);
...
const BatteryTable = ({ batteries, isSelected, toggleSelection }: any) => (
        ...
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead className="text-xs text-gray-800 dark:text-white uppercase bg-gray-100 dark:bg-gray-700">
                <tr>
                    ...
                    <th scope="col" className="text-left px-6 py-3">Última Carga</th>
                </tr>
            </thead>
            <tbody>
                {batteries.length > 0 ? batteries.map((battery: BatteryRecord) => (
                    <tr key={battery.id} className="bg-white dark:text-white border-b dark:bg-gray-900 dark:border-gray-700 hover:bg-gray-50">
                        <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            <div className="flex items-center">
                                <input
                                    checked={isSelected(battery.id)}
                                    onChange={() => toggleSelection(battery.id)}
                                    value={battery.id}
                                    type="checkbox"
                        ...
                        <td className="px-6 py-4">
                            <div className="flex items-center text-gray-900 dark:text-white">
                                <div className={statusClassname[battery.status.style_key]}></div> {battery.status.title}
                            </div>
                        </td>
                        <td className="text-left px-6 py-4">{battery.name}</td>
                        <td className="text-left px-6 py-4">{battery.manufacturer}</td>
                        <td className="text-left px-6 py-4">{battery.model}</td>
                        <td className="text-left px-6 py-4">{battery.serial_number}</td>
                        <td className="text-left px-6 py-4">{battery.last_charge}</td>
                    </tr>
                )) : (
