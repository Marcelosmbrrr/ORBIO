...
export const PilotStep: React.FC<{ selection: { id: string }[], setData: Function }> = ({ selection, setData }) => {

    const { pilots, queryParams = null } = usePage().props as any;

    const data = pilots.data;
    const meta = pilots.meta;
    ...
    const onSelect = React.useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (recordIndex !== -1) {
            ...
            const record = data.find((pilot: any) => pilot.id === selectedRecordId);
            if (record) {
    ...
    return (
        <>
            <div className='mb-5'>
                <h2 className="mb-4 text-md font-bold text-gray-900 dark:text-white">Piloto (opcional)</h2>
...
const PilotsTable = ({ data, processing, onSelect, isRowSelected }: any) => (
    <div className="mt-2 overflow-x-auto sm:rounded-lg">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                {!processing && data.length > 0 && data.map((pilot: any) => (
                    <tr key={pilot.id} className="bg-white dark:text-white border-b dark:bg-gray-900 dark:border-gray-700 hover:bg-gray-50">
                        <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            <div className="flex items-center">
                                <input
                                    checked={isRowSelected(pilot.id)}
                                    onChange={onSelect}
                                    value={pilot.id}
                                    type="checkbox"
                                    className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
